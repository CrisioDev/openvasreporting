    ...
    for f_file in input_files:
        ...
        for vuln in root.findall(".//results/result"):
            ...
            if vuln_references_temp is None:
                ...
                vuln_references = []
                for vuln_ref in vuln_references_temp.findall("./ref"):
                    if vuln_ref is None or vuln_ref.get("type").lower() == "cve" or (
                            vuln_ref.get("type").lower() == "cert-bund") or (
                            vuln_ref.get("type").lower() == "dfn-cert"):
                        vuln_ref = ""
                    else:
                        vuln_references.append(vuln_ref.get("id").lower().replace("url:", "\n"))
            ...
            vuln_qod = int(vuln.find("./qod").find("./value").text)
            ...
            temp_host_store.add_host_vuln(vuln_id, name=vuln_name, threat=vuln_threat, tags=vuln_tags, cvss=vuln_cvss,
                                          cves=vuln_cves, certs=vuln_certs, references=vuln_references,
                                          family=vuln_family,
                                          level=vuln_level, result=vuln_result, port=vuln_port, qod=vuln_qod)
...
def affected_parser(input_files, min_level=Config.levels()["n"]):
    if not isinstance(input_files, list):
        raise TypeError("Expected list, got '{}' instead".format(type(input_files)))
    else:
        for file in input_files:
    ...
    for f_file in input_files:
        ...
        for vuln in root.findall(".//results/result"):
            ...
            #try:
            vuln_detection = vuln.find("./detection")
            if vuln_detection is not None:
                vuln_detection_result = vuln_detection.find("./result")
                for detail in vuln_detection_result.findall(".//details/detail"):
                    if detail.find("./name").text == "product":
                        vuln_product = detail.find("./value")

            if vuln_product or vuln_product == "":
                continue

            logging.debug(vuln_product.text)
            # --------------------
            # VULN_NAME
            vuln_name = nvt_tmp.find("./name").text
            ...
            vuln_tags_temp = vuln_tags_text.split('|')
            vuln_tags = dict(tag.split('=', 1) for tag in vuln_tags_temp)
            vuln_affected = vuln_tags.get('affected', '')
            vuln_solution = vuln_tags.get('solution', '')
            #logging.debug("* vuln_tags:\t{}".format(vuln_tags))  # DEBUG
            ...
            try:
                # check if host exists in hosts
                temp_host_store = affected_hosts[vuln_host]
            except KeyError:
                # else create host in hosts
                temp_host_store = Host(vuln_host, vuln_host_name)

            vuln_affected = vuln_product.text

            temp_host_store.add_host_affected(vuln_affected, solution=vuln_solution, level= vuln_level, qod= vuln_qod)

            affected_hosts[vuln_host] = temp_host_store

    return list(affected_hosts.values())
